use crate::{pos_from, Pos};
use aoc_parse::{parser, prelude::any_char, prelude::char_of, prelude::lines, Parser};
...
impl Grid {
    ...
    pub fn take_all(&mut self, c: char) -> HashSet<Pos> {
        self.map.remove(&c).unwrap_or_else(|| HashSet::new())
    }
...
where
    ...
    Q: Parser,
{
    parser!(g:lines(({ k:keep => Some(k), i:ignored => None })+) => {
...
pub fn grid_of(chars: &'static str) -> impl Parser<Output = Grid> {
